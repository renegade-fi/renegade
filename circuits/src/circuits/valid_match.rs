...
use crate::{
    gadgets::{
        poseidon::{
            MatchHashInput, PoseidonSpongeWrapperVar, PoseidonVectorHashGadget, WalletHashInput,
        },
        wallet_match::MatchGadget,
    },
    types::{MatchResult, SystemField, SystemPairingEngine, Wallet, WalletVar},
};
...
impl ValidMatchCircuit {
    pub fn new(
        ...
        let wrapped_type = RefCell::new(ValidMatchCircuitImpl::<SystemField>::new(
            wallet1,
            wallet2,
            wallet1_hash,
            wallet2_hash,
    ...
    pub fn create_proving_key(
        max_balances: usize,
        max_orders: usize,
    ) -> Result<ProvingKey<SystemPairingEngine>, SynthesisError> {
        let mut rng = OsRng {};
    ...
    pub fn generate_constraints(
        ...
        self.wrapped_type.take().generate_constraints(cs)
    }
...
impl<F: PrimeField> ConstraintSynthesizer<F> for ValidMatchCircuitImpl<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        let expected_hash_1 =
            FpVar::new_input(cs.clone(), || Ok(F::from(self.wallet1_hash.clone())))?;

        let expected_hash_2 =
            FpVar::new_input(cs.clone(), || Ok(F::from(self.wallet2_hash.clone())))?;

        // Allocate the private inputs
        let wallet1_var = WalletVar::new_witness(cs.clone(), || Ok(&self.wallet1))?;

        let wallet2_var = WalletVar::new_witness(cs.clone(), || Ok(&self.wallet2))?;
        ...
        let wallet1_hash =
            PoseidonVectorHashGadget::evaluate(&wallet1_hash_input, &mut wallet1_hasher)?;
        ...
        let wallet2_hash =
            PoseidonVectorHashGadget::evaluate(&wallet2_hash_input, &mut wallet2_hasher)?;
...
mod valid_match_test {
    ...
    use crate::{
        ...
        types::{Balance, Order, OrderSide, SystemField, Wallet},
    };
    ...
    fn test_prove() {
        ...
        let circuit =
            ValidMatchCircuit::new(wallet1, wallet2, wallet1_hash.clone(), wallet2_hash.clone());
