...
pub(crate) mod edwards_tests {
    ...
    /// Sample a random field element from the field that ed25519 is defined over
    pub(crate) fn ed25519_random_felt<R: RngCore + CryptoRng>(rng: &mut R) -> BigUint {
        // Generate random bytes to fill a BigUint
        let mut bytes = vec![0u8; 32];
        rng.fill_bytes(&mut bytes);
        let random_bigint = BigUint::from_bytes_le(&bytes);

        random_bigint % ((BigUint::from(1u8) << 255) - 19u8)
    }
