...
use ark_r1cs_std::{prelude::AllocVar, uint64::UInt64, uint8::UInt8, R1CSVar};
...
impl Wallet {
    ...
    pub fn hash(&self) -> BigUint {
        ...
        let mut sponge =
            PoseidonSponge::<SystemField>::new(&PoseidonSpongeWrapperVar::default_params());
...
impl<F: PrimeField> AllocVar<Wallet, F> for WalletVar<F> {
    ...
    fn new_variable<T: Borrow<Wallet>>(
        ...
        f().and_then(|wallet| {
            ...
            let mut balances: Vec<BalanceVar<F>> = wallet
                .balances
                .iter()
                .map(|balance| BalanceVar::new_variable(cs.clone(), || Ok(balance), mode))
            ...
            for _ in 0..(wallet._max_balances - wallet.balances.len()) {
                balances.push(BalanceVar::new_variable(
                    cs.clone(),
                    || Ok(Balance::default()),
                    mode,
                )?)
            ...
            let mut orders: Vec<OrderVar<F>> = wallet
                .orders
                .iter()
                .map(|order| OrderVar::new_variable(cs.clone(), || Ok(order), mode))
            ...
            for _ in 0..(wallet._max_orders - wallet.orders.len()) {
                orders.push(OrderVar::new_variable(
                    cs.clone(),
                    || Ok(Order::default()),
                    mode,
                )?)
...
impl<F: PrimeField> AllocVar<Balance, F> for BalanceVar<F> {
    fn new_variable<T: Borrow<Balance>>(
        ...
        f().and_then(|balance| {
            let cs = cs.into();
            let mint = UInt64::new_variable(cs.clone(), || Ok(balance.borrow().mint), mode)?;

            let amount = UInt64::new_variable(cs, || Ok(balance.borrow().amount), mode)?;
...
impl<F: PrimeField> AllocVar<Order, F> for OrderVar<F> {
    fn new_variable<T: Borrow<Order>>(
        ...
        f().and_then(|order| {
            let cs = cs.into();
            let quote_mint =
                UInt64::new_variable(cs.clone(), || Ok(order.borrow().quote_mint), mode)?;

            let base_mint =
                UInt64::new_variable(cs.clone(), || Ok(order.borrow().base_mint), mode)?;
            ...
            let price = UInt64::new_variable(cs.clone(), || Ok(order.borrow().price), mode)?;

            let amount = UInt64::new_variable(cs, || Ok(order.borrow().amount), mode)?;
...
impl<F: PrimeField> AllocVar<SingleMatchResult, F> for SingleMatchResultVar<F> {
    fn new_variable<T: Borrow<SingleMatchResult>>(
        ...
        f().and_then(|single_match| {
            ...
            Ok(Self {
                buy_side1: MatchVariable::new_variable(
                    cs.clone(),
                    || Ok(single_match.buy_side1.clone()),
                    mode,
                )?,
                sell_side1: MatchVariable::new_variable(
                    cs.clone(),
                    || Ok(single_match.sell_side1.clone()),
                    mode,
                )?,
                buy_side2: MatchVariable::new_variable(
                    cs.clone(),
                    || Ok(single_match.buy_side2.clone()),
                    mode,
                )?,
                sell_side2: MatchVariable::new_variable(
                    cs,
                    || Ok(single_match.sell_side2.clone()),
                    mode,
                )?,
            })
        })
