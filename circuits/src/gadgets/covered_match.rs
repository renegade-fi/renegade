...
use crate::types::{BalanceVar, MatchVariable, OrderVar, SingleMatchResultVar};

use super::{poseidon::u64_to_field_element, util::GreaterThanEqGadget};
...
impl<F: PrimeField> ValidMatchGadget<F> {
    pub fn enforce_valid(single_match: &SingleMatchResultVar<F>) -> Result<(), SynthesisError> {
    ...
    pub fn enforce_valid_balances(
        ...
        single_match.sell_side1.mint.enforce_equal(&balance1.mint)?;

        GreaterThanEqGadget::greater_than_u64(&balance1.amount, &single_match.sell_side1.amount)?
            .enforce_equal(&Boolean::TRUE)?;

        single_match.sell_side2.mint.enforce_equal(&balance2.mint)?;

        GreaterThanEqGadget::greater_than_u64(&balance2.amount, &single_match.sell_side2.amount)?
            .enforce_equal(&Boolean::TRUE)?;
    ...
    pub fn enforce_valid_orders(
        ...
        let execution_price =
            FpVar::conditionally_select(&order1_buy, &execution_price2, &execution_price1)?;
    ...
    fn enforce_order_at_price_and_amount(
        ...
        let buy_mint = UInt64::conditionally_select(&is_buy, &order.base_mint, &order.quote_mint)?;

        let sell_mint = UInt64::conditionally_select(&is_buy, &order.quote_mint, &order.base_mint)?;
        ...
        let order_amount =
            UInt64::conditionally_select(&is_buy, &buy_side_match.amount, &sell_side_match.amount)?;
        ...
        Boolean::conditionally_select(&is_buy, &less_than_order, &greater_than_order)?
            .enforce_equal(&Boolean::TRUE)?;
