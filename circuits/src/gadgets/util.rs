...
impl<F: PrimeField> GreaterThanEqGadget<F> {
    pub fn greater_than(a: &FpVar<F>, b: &FpVar<F>) -> Result<Boolean<F>, SynthesisError> {
        let diff = a - b;
    ...
    pub fn greater_than_u64(a: &UInt64<F>, b: &UInt64<F>) -> Result<Boolean<F>, SynthesisError> {
...
impl<F: PrimeField> MinGadget<F> {
    pub fn min(a: FpVar<F>, b: FpVar<F>) -> Result<FpVar<F>, SynthesisError> {
    ...
    pub fn min_uint64(a: UInt64<F>, b: UInt64<F>) -> Result<UInt64<F>, SynthesisError> {
        let a_fp = Boolean::le_bits_to_fp_var(&a.to_bits_le())?;
        let b_fp = Boolean::le_bits_to_fp_var(&b.to_bits_le())?;
...
impl<F: PrimeField> MaskGadget<F> {
    pub fn mask_uint64(a: &UInt64<F>, cond: &Boolean<F>) -> Result<UInt64<F>, SynthesisError> {
    ...
    pub fn mask_uint8(a: &UInt8<F>, cond: &Boolean<F>) -> Result<UInt8<F>, SynthesisError> {
