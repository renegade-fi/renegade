...
use ark_r1cs_std::{
    bits::{boolean::Boolean, uint64::UInt64, uint8::UInt8},
    prelude::EqGadget,
    ToBitsGadget,
};
...
use crate::{
    gadgets::util::{GreaterThanEqGadget, MaskGadget, MinGadget},
    types::{MatchResultVariable, MatchVariable, WalletVar},
};
    ...
    pub fn compute_matches(
        ...
        for i in 0..wallet1.orders.len() {
            for j in 0..wallet2.orders.len() {
                ...
                let opposite_sides = order1.side.xor(&order2.side)?.is_eq(&UInt8::constant(1))?;
                ...
                let execution_price =
                    UInt64::addmany(&[order1.price.clone(), order2.price.clone()])?
                        .rotr(1)
                        .xor(&UInt64::<F>::constant(1 << 63))?;

                let base_swapped =
                    MinGadget::min_uint64(order1.amount.clone(), order2.amount.clone())?;
                ...
                let quote_mint_masked =
                    MaskGadget::mask_uint64(&order1.quote_mint, &aggregated_checks)?;
                let base_mint_masked =
                    MaskGadget::mask_uint64(&order1.base_mint, &aggregated_checks)?;

                let base_swapped_masked =
                    MaskGadget::mask_uint64(&base_swapped, &aggregated_checks)?;
                let quote_swapped_masked =
                    MaskGadget::mask_uint64(&quote_swapped, &aggregated_checks)?;
                ...
                result.matches1.push(MatchVariable {
                ...
                result.matches2.push(MatchVariable {
...
mod overlap_test {
    ...
    fn setup_prices<F: PrimeField>(
        ...
        // Buy side
        let side1_var = UInt8::new_witness(ark_relations::ns!(cs, "side1"), || Ok(0)).unwrap();
        let price1_var =
            UInt64::new_witness(ark_relations::ns!(cs, "price1"), || Ok(buy_price)).unwrap();

        // Sell side
        let side2_var = UInt8::new_witness(ark_relations::ns!(cs, "side2"), || Ok(1)).unwrap();
        let price2_var =
            UInt64::new_witness(ark_relations::ns!(cs, "price2"), || Ok(sell_price)).unwrap();
    ...
    fn test_overlap_gadget_no_overlap() {
        ...
        let result_var =
            OverlapGadget::is_overlapping(&side1_var, &price1_var, &side2_var, &price2_var)
                .unwrap();
...
mod match_test {
    ...
    use crate::types::{Match, Order, OrderSide, SystemField, Wallet};

    fn has_nonzero_match(matches_list: &[Match]) -> bool {
        matches_list.iter().any(|match_res| {
            match_res.amount != 0 || match_res.mint != 0 || match_res.side == OrderSide::Sell
            // Sell side is 1
        })
    }
    ...
    fn test_match_correct_match() {
        ...
        assert!(matches1[0].eq(&Match {
            mint: base_mint,
            amount: 2,
            side: OrderSide::Sell
        }));
        assert!(matches1[1].eq(&Match {
            mint: quote_mint,
            amount: 30,
            side: OrderSide::Buy
        }));

        assert!(matches2[0].eq(&Match {
            mint: base_mint,
            amount: 2,
            side: OrderSide::Buy
        }));
        assert!(matches2[1].eq(&Match {
            mint: quote_mint,
            amount: 30,
            side: OrderSide::Sell
        }));
...
mod proof_test {
    ...
    use crate::types::{Order, OrderSide, Wallet, WalletVar};
    ...
    impl<F: PrimeField> ConstraintSynthesizer<F> for DummyCircuit<F> {
        fn generate_constraints(
            ...
            let res = MatchGadget::compute_matches(&wallet1_var, &wallet2_var).unwrap();

            res.matches1[0]
                .amount
                .is_eq(&UInt64::constant(20))
                .unwrap()
    ...
    fn test_prove_wallet() {
        ...
        let verification_result = verify_proof(&verifying_key, &proof, &Vec::new()[..]).unwrap();
