//! Tests the process of proving and verifying an `INTENT ONLY BOUNDED
//! SETTLEMENT` circuit

use circuit_types::PlonkCircuit;
use circuit_types::traits::{CircuitBaseType, SingleProverCircuit};
use circuits::zk_circuits::settlement::intent_only_bounded_settlement::IntentOnlyBoundedSettlementCircuit;
use circuits::zk_circuits::settlement::intent_only_bounded_settlement::test_helpers::create_witness_statement;
...
pub fn bench_apply_constraints(c: &mut Criterion) {
    ...
    // Run the benchmark
    let mut group = c.benchmark_group("intent_only_bounded_settlement");
    let benchmark_id = BenchmarkId::new("constraint-generation", "");

    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            IntentOnlyBoundedSettlementCircuit::apply_constraints(
                witness_var.clone(),
                statement_var.clone(),
                &mut cs,
            )
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<IntentOnlyBoundedSettlementCircuit>(&witness, &statement).unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<IntentOnlyBoundedSettlementCircuit>(&statement, &proof)
                .unwrap();
        });
    });
}
...
criterion_group! {
    name = intent_only_bounded_settlement;
    config = Criterion::default().sample_size(10);
    targets = bench_apply_constraints, bench_prover, bench_verifier
}
criterion_main!(intent_only_bounded_settlement);
