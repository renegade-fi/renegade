//! Tests the process of proving and verifying a `NEW OUTPUT BALANCE VALIDITY`
//! circuit
...
use circuits::zk_circuits::v2::validity_proofs::new_output_balance::NewOutputBalanceValidityCircuit;
use circuits::zk_circuits::v2::validity_proofs::new_output_balance::test_helpers::create_witness_statement;
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<NewOutputBalanceValidityCircuit>(
                witness.clone(),
                statement.clone(),
            )
            .unwrap();
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    let (witness, statement) = create_witness_statement();
    let proof =
        singleprover_prove::<NewOutputBalanceValidityCircuit>(witness, statement.clone()).unwrap();
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<NewOutputBalanceValidityCircuit>(statement.clone(), &proof)
                .unwrap();
        });
    });
}
...
criterion_group! {
    name = new_output_balance_validity;
    config = Criterion::default().sample_size(10);
    targets = bench_apply_constraints, bench_prover, bench_verifier
}
criterion_main!(new_output_balance_validity);
