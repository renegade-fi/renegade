//! Tests the process of proving and verifying a `VALID NOTE REDEMPTION` circuit
#![allow(incomplete_features)]
#![allow(missing_docs)]

use circuit_types::PlonkCircuit;
use circuit_types::traits::{CircuitBaseType, SingleProverCircuit};
use circuits::zk_circuits::v2::fees::valid_note_redemption::SizedValidNoteRedemption;
use circuits::zk_circuits::v2::fees::valid_note_redemption::test_helpers::create_dummy_witness_statement;
...
pub fn bench_apply_constraints(c: &mut Criterion) {
    ...
    // Run the benchmark
    let mut group = c.benchmark_group("valid_note_redemption");
    let benchmark_id = BenchmarkId::new("constraint-generation", format!("({MERKLE_HEIGHT})"));

    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            SizedValidNoteRedemption::apply_constraints(
                witness_var.clone(),
                statement_var.clone(),
                &mut cs,
            )
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<SizedValidNoteRedemption>(witness.clone(), statement.clone())
                .unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    let (witness, statement) = create_dummy_witness_statement();
    let proof = singleprover_prove::<SizedValidNoteRedemption>(witness, statement.clone()).unwrap();
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<SizedValidNoteRedemption>(statement.clone(), &proof)
                .unwrap();
        });
    });
}
...
criterion_main!(valid_note_redemption);
