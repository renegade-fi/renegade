...
use circuits::zk_circuits::v2::valid_balance_create::test_helpers::create_dummy_witness_statement;
use circuits::zk_circuits::v2::valid_balance_create::{
    SizedValidBalanceCreate, SizedValidBalanceCreateStatement, SizedValidBalanceCreateWitness,
};
...
pub fn bench_apply_constraints(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            SizedValidBalanceCreate::apply_constraints(
                witness_var.clone(),
                statement_var.clone(),
                &mut cs,
            )
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<SizedValidBalanceCreate>(witness.clone(), statement.clone())
                .unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    let (witness, statement) = create_witness_statement();
    let proof = singleprover_prove::<SizedValidBalanceCreate>(witness, statement.clone()).unwrap();
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<SizedValidBalanceCreate>(statement.clone(), &proof)
                .unwrap();
        });
    });
}
