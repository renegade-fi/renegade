...
fn get_asset_and_volume(mint: &BigUint, amount: &BigUint) -> Result<(String, f64), String> {
...
fn maybe_record_volume(mint: &BigUint, amount: &BigUint, volume_metric_name: &'static str) {
...
pub(crate) fn maybe_record_transfer_metrics(transfer: &Option<ExternalTransfer>) {
    if let Some(transfer) = transfer.as_ref() {
        ...
        maybe_record_volume(&transfer.mint, &transfer.amount, volume_metric);
    }
}

/// Record the volume of base/quote assets moved in a match
pub(crate) fn maybe_record_match_volume(match_result: &MatchResult) {
    maybe_record_volume(
        &match_result.base_mint,
        &BigUint::from(match_result.base_amount),
        MATCH_BASE_VOLUME_METRIC,
    );
    maybe_record_volume(
        &match_result.quote_mint,
        &BigUint::from(match_result.quote_amount),
