...
impl SettleMatchInternalTask {
    ...
    async fn prove_match_settle(&mut self) -> Result<(), SettleMatchInternalTaskError> {
        ...
        let party0_commitment_link = self.order1_validity_witness.copy_commitment_linking_hint();
        let party1_commitment_link = self.order2_validity_witness.copy_commitment_linking_hint();

        // Enqueue a job with the proof generation module
        let job = ProofJob::ValidMatchSettleSingleprover {
            witness,
            statement,
    ...
    async fn submit_match(&mut self) -> Result<(), SettleMatchInternalTaskError> {
        ...
        let tx = self
            ...
            .process_match_settle(
                &self.order1_proof,
                &self.order2_proof,
                self.match_bundle.clone().unwrap(),
            )
            .await?;
        self.tx = Some(tx);
        Ok(())
    ...
    async fn update_state(&self) -> Result<(), SettleMatchInternalTaskError> {
        ...
        let party0_public_shares = &match_bundle.statement.party0_modified_shares;
        let party0_private_shares =
            &self.order1_validity_witness.reblind_witness.reblinded_wallet_private_shares;
        let party1_public_shares = &match_bundle.statement.party1_modified_shares;
