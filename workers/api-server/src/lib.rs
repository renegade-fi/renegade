...
fn authenticate_wallet_request(
    ...
    //     .as_bytes();
    // let sig_expiration =
    // headers.get(RENEGADE_SIG_EXPIRATION_HEADER_NAME).ok_or_else(|| {
    //     ApiServerError::HttpStatusCode(
    //         StatusCode::BAD_REQUEST,
    ...
    // // Recover a public key from the byte packed scalar representing the
    // public key let root_key: PublicKey = pk_root.into();
