...
use crate::{
    global_metrics::MATCHING_ENGINE_INFLIGHT_JOBS,
    labels::{
        ...
        MATCHING_ENGINE_JOB_DURATION_MS_METRIC, MATCHING_ENGINE_JOB_TYPE_TAG,
...
///
/// The in-flight gauge uses the atomic [`Gauge`](crate::gauge::Gauge) wrapper
/// (`.set()` of an internally tracked count) because the StatsD exporter drops
/// raw gauge increment/decrement deltas and only exports absolute `.set()`
/// values — the queue-length gauge below works for the same reason.
pub fn record_matching_engine_job_started(queue_len: usize) {
    ...
    MATCHING_ENGINE_INFLIGHT_JOBS.increment(1.0);
}

/// Record that a matching engine job has finished, with its handling duration.
///
/// Decrements the in-flight gauge and records the duration histogram, tagged by
/// `job_type` (`internal` | `external_quote` | `external_settle`) so the latency
/// tail can be attributed to a job kind.
pub fn record_matching_engine_job_finished(duration_ms: f64, job_type: &str) {
    MATCHING_ENGINE_INFLIGHT_JOBS.decrement(1.0);
    let labels = [(MATCHING_ENGINE_JOB_TYPE_TAG.to_string(), job_type.to_string())];
    metrics::histogram!(MATCHING_ENGINE_JOB_DURATION_MS_METRIC, &labels).record(duration_ms);
}
