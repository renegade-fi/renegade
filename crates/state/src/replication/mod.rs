...
// notifications::OpenNotifications,         storage::db::DB,
//     };
...
//             mock::{MockNetworkNode, SwitchReceiver, SwitchSender,
// new_switch_queue},         },
...
//         let sm = StateMachine::new(sm_config, notifications,
// applicator).await.unwrap();         let log = LogStore::new(db);

//         (sm, log)
//     }

//     /// Get the config for a mock raft
//     pub fn mock_raft_config(initial_nodes: Vec<NodeId>, delay: u64) ->
// RaftClientConfig {         // Use mock peer ids for each node
//         let initial_nodes =
//             initial_nodes.into_iter().map(|nid| (nid,
// RaftNode::default())).collect();         let heartbeat_interval = delay + 5;
...
//             Self::create_raft(1, 0 /* network_delay_ms */, true /* init
// */).await         }
...
//             Self::create_raft(n_nodes, 0 /* network_delay_ms */, true /* init
// */).await         }

//         /// Create a mock raft network with `n_nodes` nodes and return the
// rafts         /// in use
//         pub async fn create_raft(n_nodes: usize, network_delay_ms: u64, init:
// bool) -> Self {             let (send, recv) = new_switch_queue();
...
//                 let mock_net = MockNetworkNode::new_with_delay(send.clone(),
// network_delay_ms);                 let applicator = mock_applicator();
//                 let db = applicator.config.db.clone();
//                 let notifications = OpenNotifications::new();
//                 let sm_conf = StateMachineConfig::new(db.path().to_string());
//                 let sm = StateMachine::new(sm_conf, notifications,
// applicator).await.unwrap();

//                 let client = RaftClient::new(conf, db.clone(), mock_net,
// sm).await.unwrap();                 nodes.insert(i, MockRaftNode::new(client,
// db));             }
...
//         async fn run(mut queue: SwitchReceiver, rafts:
// AsyncShared<HashMap<NodeId, MockRaftNode>>) {             loop {
...
//         ///
//         /// This method panics on errors in the receiver at the moment, this
// is         /// okay for tests, but the main implementation should handle
// failures         async fn forward_req(client: RaftClient, req: RaftRequest)
...
//             let client = RaftClient::new(config, db.clone(), mock_net,
// sm).await.unwrap();             let node = MockRaftNode::new(client, db);
//             self.rafts.write().await.insert(nid, node);
//         }

//         /// Remove a node from the network, does not propose a state
// transition         pub async fn remove_node(&self, nid: NodeId) {
