...
impl Token {
    ...
    pub fn get_exchange_ticker(&self, exchange: Exchange) -> Option<String> {
        ...
        if !self.is_named() {
            panic!("Tried to get_exchange_ticker({exchange}) for an unnamed Token.");
        }
    ...
    /// Converts the amount of the token as an f64 to a `u128`, accounting
    /// for the associated number of decimals.
    ///
    /// Note that due to conversion from f64, the result may lose precision.
    pub fn convert_from_decimal(&self, amount: f64) -> u128 {
        let decimals = self.get_decimals().unwrap_or_default();
        let decimal_adjustment = 10u128.pow(decimals as u32);
