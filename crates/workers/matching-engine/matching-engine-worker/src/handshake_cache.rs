...
pub(super) type SharedHandshakeCache = AsyncShared<HandshakeCache>;
...
pub struct HandshakeCache {
    ...
    /// first
    lru_cache: LruCache<(OrderId, OrderId), HandshakeCacheState>,
}
...
impl HandshakeCache {
    ...
    fn cache_tuple(o1: OrderId, o2: OrderId) -> (OrderId, OrderId) {
        let first_entry = min(o1, o2);
        let second_entry = max(o1, o2);
        (first_entry, second_entry)
    }

    /// Caches an entry
    pub fn mark_completed(&mut self, o1: OrderId, o2: OrderId) {
    ...
    pub fn mark_invisible(&mut self, o1: OrderId, o2: OrderId) {
    ...
    pub fn contains(&self, o1: OrderId, o2: OrderId) -> bool {
...
mod handshake_cache_tests {
    ...
    fn test_lru_policy() {
        ...
        let id1 = OrderId::from_u128_le(1);
        let id2 = OrderId::from_u128_le(2);
        let id3 = OrderId::from_u128_le(3);
        cache.mark_completed(id1, id1);
        cache.mark_completed(id2, id2);
        cache.mark_completed(id3, id3);

        assert!(!cache.contains(id1, id1));
        assert!(cache.contains(id2, id2));
        assert!(cache.contains(id3, id3));
    }
    ...
    fn test_cache_ordering() {
        ...
        let id4 = OrderId::from_u128_le(4);
        let id5 = OrderId::from_u128_le(5);
        let id6 = OrderId::from_u128_le(6);
        let id7 = OrderId::from_u128_le(7);
        // Try the smaller value first
        cache.mark_completed(id4, id5);
        assert!(cache.contains(id4, id5));
        assert!(cache.contains(id5, id4));

        // Try the larger value first
        cache.mark_completed(id7, id6);
        assert!(!cache.contains(id4, id5));
        assert!(cache.contains(id6, id7));
        assert!(cache.contains(id7, id6));
    }
}
