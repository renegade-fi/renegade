...
pub struct HandshakeScheduler {
    ...
    job_sender: MatchingEngineWorkerQueue,
...
impl HandshakeScheduler {
    ...
    pub fn new(job_sender: MatchingEngineWorkerQueue, state: State, cancel: CancelChannel) -> Self {
            ...
            tokio::select! {
                ...
                _ = tokio::time::sleep(refresh_interval) => {
                    ...
                    if let Some(order) = self.state.choose_handshake_order().await.ok().flatten() {
                        if let Err(e) = self
                            .job_sender
                            .send(MatchingEngineWorkerJob::PerformHandshake { order })
