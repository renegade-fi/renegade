...
pub struct SettleExternalMatchTask {
    ...
    /// The calldata bundle generated by the task
    pub calldata_bundle: Option<(BoundedMatchResult, FeeRates, TransactionRequest)>,
...
impl SettleExternalMatchTask {
    ...
    async fn generate_calldata(&self) -> Result<()> {
    ...
    async fn forward_bounded_match(&self) -> Result<()> {
        ...
        let (match_result, fee_rates, transaction) = self.calldata_bundle.clone().unwrap();
        let message =
