...
use circuits_core::zk_circuits::validity_proofs::intent_only_first_fill::IntentOnlyFirstFillValidityCircuit;
use circuits_core::zk_circuits::validity_proofs::intent_only_first_fill::test_helpers::create_witness_statement;
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<IntentOnlyFirstFillValidityCircuit>(&witness, &statement).unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<IntentOnlyFirstFillValidityCircuit>(&statement, &proof)
                .unwrap();
        });
    });
}
