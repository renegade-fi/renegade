...
use circuits_core::zk_circuits::validity_proofs::intent_only::IntentOnlyValidityCircuit;
use circuits_core::zk_circuits::validity_proofs::intent_only::test_helpers::create_witness_statement;
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<IntentOnlyValidityCircuit<MERKLE_HEIGHT>>(&witness, &statement)
                .unwrap();
        });
    });
}
