...
use circuits_core::zk_circuits::fees::valid_note_redemption::SizedValidNoteRedemption;
use circuits_core::zk_circuits::fees::valid_note_redemption::test_helpers::create_dummy_witness_statement;
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    let (witness, statement) = create_dummy_witness_statement();
    let proof = singleprover_prove::<SizedValidNoteRedemption>(&witness, &statement).unwrap();
