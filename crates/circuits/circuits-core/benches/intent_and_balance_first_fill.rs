//! Tests the process of proving and verifying an `INTENT AND BALANCE FIRST FILL
//! VALIDITY` circuit
...
use circuits_core::zk_circuits::validity_proofs::intent_and_balance_first_fill::SizedIntentAndBalanceFirstFillValidityCircuit;
use circuits_core::zk_circuits::validity_proofs::intent_and_balance_first_fill::test_helpers::create_witness_statement;
...
pub fn bench_apply_constraints(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            SizedIntentAndBalanceFirstFillValidityCircuit::apply_constraints(
                witness_var.clone(),
                statement_var.clone(),
                &mut cs,
            )
...
criterion_group! {
    name = intent_and_balance_first_fill_validity;
    config = Criterion::default().sample_size(10);
    targets = bench_apply_constraints, bench_prover, bench_verifier
}
criterion_main!(intent_and_balance_first_fill_validity);
