...
use circuits_core::zk_circuits::fees::valid_public_protocol_fee_payment::SizedValidPublicProtocolFeePayment;
use circuits_core::zk_circuits::fees::valid_public_protocol_fee_payment::test_helpers::create_dummy_witness_statement;
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<SizedValidPublicProtocolFeePayment>(&witness, &statement).unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<SizedValidPublicProtocolFeePayment>(&statement, &proof)
                .unwrap();
        });
    });
}
