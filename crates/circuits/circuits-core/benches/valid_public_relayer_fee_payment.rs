//! Tests the process of proving and verifying a `VALID PUBLIC RELAYER FEE
//! PAYMENT` circuit
...
use circuits_core::zk_circuits::fees::valid_public_relayer_fee_payment::SizedValidPublicRelayerFeePayment;
use circuits_core::zk_circuits::fees::valid_public_relayer_fee_payment::test_helpers::create_dummy_witness_statement;
...
pub fn bench_apply_constraints(c: &mut Criterion) {
    ...
    // Run the benchmark
    let mut group = c.benchmark_group("valid_public_relayer_fee_payment");
    let benchmark_id = BenchmarkId::new("constraint-generation", format!("({MERKLE_HEIGHT})"));

    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            SizedValidPublicRelayerFeePayment::apply_constraints(
                witness_var.clone(),
                statement_var.clone(),
                &mut cs,
            )
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<SizedValidPublicRelayerFeePayment>(&witness, &statement).unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<SizedValidPublicRelayerFeePayment>(&statement, &proof)
                .unwrap();
        });
    });
}
...
criterion_main!(valid_public_relayer_fee_payment);
