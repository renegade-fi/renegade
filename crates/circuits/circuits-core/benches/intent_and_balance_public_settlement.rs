//! Tests the process of proving and verifying an `INTENT AND BALANCE PUBLIC
//! SETTLEMENT` circuit
...
use circuits_core::zk_circuits::settlement::intent_and_balance_public_settlement::IntentAndBalancePublicSettlementCircuit;
use circuits_core::zk_circuits::settlement::intent_and_balance_public_settlement::test_helpers::create_witness_statement;
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<IntentAndBalancePublicSettlementCircuit>(&witness, &statement)
                .unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    let (witness, statement) = create_witness_statement::<MERKLE_HEIGHT>();
    let proof = singleprover_prove::<IntentAndBalancePublicSettlementCircuit>(&witness, &statement)
        .unwrap();
...
criterion_main!(intent_and_balance_public_settlement);
