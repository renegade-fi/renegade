...
use circuits_core::zk_circuits::fees::valid_private_relayer_fee_payment::SizedValidPrivateRelayerFeePayment;
use circuits_core::zk_circuits::fees::valid_private_relayer_fee_payment::test_helpers::create_dummy_witness_statement;
...
pub fn bench_prover(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            singleprover_prove::<SizedValidPrivateRelayerFeePayment>(&witness, &statement).unwrap();
        });
    });
}
...
pub fn bench_verifier(c: &mut Criterion) {
    ...
    group.bench_function(benchmark_id, |b| {
        b.iter(|| {
            verify_singleprover_proof::<SizedValidPrivateRelayerFeePayment>(&statement, &proof)
                .unwrap();
        });
    });
}
